...
public class StudentService extends Services implements StudentDao {
	...
	public Boolean validateStudent(String sEmail, String sPass) {
		...
		try {
			...
			if (foundStudent.getsPass().equals(sPass))
				return true;
	...
	public void registerStudentToCourse(String sEmail, int cid) throws Exception {
		...
		try {
			List studentEmails = em
					.createNativeQuery("SELECT Student_SEMAIL" + " FROM student_course sc"
							+ " WHERE sc.Student_SEMAIL = ? AND sc.sCourses_CID = ?")
					.setParameter(1, sEmail).setParameter(2, cid).getResultList();
			...
			if (studentEmails.isEmpty()) {
				...
				int newRegister = em.createNativeQuery("INSERT INTO student_course VALUES (?, ?)")
						.setParameter(1, sEmail).setParameter(2, cid).executeUpdate();
				...
				System.out.println("Student is already registered for Course #" + cid);
